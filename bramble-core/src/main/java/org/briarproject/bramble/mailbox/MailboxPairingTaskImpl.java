...
class MailboxPairingTaskImpl implements MailboxPairingTask {
	...
	private final List<Consumer<MailboxPairingState>> observers =
			new ArrayList<>();
	@GuardedBy("lock")
	private MailboxPairingState state = new MailboxPairingState.Pairing();
	...
	public void run() {
		try {
			...
			onMailboxError(new MailboxPairingState.InvalidQrCode());
		} catch (MailboxAlreadyPairedException e) {
			onMailboxError(new MailboxPairingState.MailboxAlreadyPaired());
		} catch (IOException e) {
			onMailboxError(new MailboxPairingState.ConnectionError());
		} catch (ApiException | DbException e) {
			onMailboxError(new MailboxPairingState.AssertionError());
		}
	}
	...
	private void onMailboxError(MailboxPairingState state) {
		synchronized (lock) {
			this.state = state;
			notifyObservers();
		}
